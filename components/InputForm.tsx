    ...
    <form onSubmit={handleSubmit} className="w-full space-y-5">
      ...
      <div>
        ...
        <div className={`panel flex items-center px-4 py-3.5 gap-3 transition-colors duration-150 ${
          ...
          <svg className="w-4 h-4 text-[#3a4e63] flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[#e8edf3] placeholder-[#3a4e63] text-base outline-none"
            autoComplete="off"
          />
        </div>
        <div className="flex flex-wrap gap-2 mt-2.5">
          <span className="mono text-[#3a4e63] text-xs self-center">try:</span>
          {['wireless earbuds', 'pet grooming brush', 'yoga mat'].map((ex) => (
            <button
              key={ex}
              type="button"
              onClick={() => setKeyword(ex)}
              className="tag tag-dim hover:border-[#2d3f55] hover:text-[#8499b0] transition-all duration-150 cursor-pointer"
            >
              {ex}
            </button>
          ))}
        ...
        {[
          ...
          <div key={field.id}>
            ...
            <div className="panel px-4 py-3.5 relative">
              ...
              <svg className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-[#3a4e63] pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor">
